...
describe('useCampusScope', () => {
  ...
  it('總管理者看得到五校並預選第一校', () => {
    ...
    auth.user = { id: '1', email: 'a@b', role: 'super_admin', is_active: true, campus_keys: [], line_linked: false }
  ...
  it('校區管理者只看到自己的校區，換帳號時選取值跟著修正', async () => {
    const auth = useAuthStore()
    auth.user = { id: '2', email: 'c@d', role: 'campus_admin', is_active: true, campus_keys: ['renwu'], line_linked: false }
    const scope = useCampusScope()
    expect(scope.visibleCampusKeys.value).toEqual(['renwu'])
    expect(scope.selected.value).toBe('renwu')

    auth.user = { id: '3', email: 'e@f', role: 'campus_admin', is_active: true, campus_keys: ['minghua', 'chongde'], line_linked: false }
    await nextTick()
