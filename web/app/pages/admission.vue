...
// 入學資訊（2026-09-24）：舊官網「常春藤入學」四個分頁（寶貝入學流程／新生入園須知／
// 收退費辦法／分班表）移植而來。內容由後台 admission_content 管理；區塊大標固定在這裡，
// 因為標題字型是子集（CLAUDE.md「字型子集」）。分班對照依生日規則計算，見 admission-classes.ts。
...
const admission = computed(() => data.value!.content.admission)
...
<template>
  <div v-if="data">
    ...
    <main id="main" tabindex="-1" class="admission">
      ...
      <!-- 新生入園須知 -->
      <section v-if="admission.phases.length || hasNewcomerExtras" id="newcomer" class="section campuses">
        <div class="container">
          <div class="section-heading">
            <div>
