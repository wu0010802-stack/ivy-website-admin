...
.campus-tabs{display:flex;justify-content:center;width:min(100%,720px);border-block:1px solid var(--heading-rule)}
.campus-tabs button{position:relative;flex:1;min-width:0;min-height:60px;padding:12px 20px;border:0;border-radius:0;background:transparent;font-size:15px;letter-spacing:.065em;white-space:nowrap;color:var(--muted);transition:background .2s,color .2s}
.campus-tabs button[aria-selected=true]{color:var(--heading-ink);font-weight:600}
.campus-tabs button[aria-selected=true]::after{content:'';position:absolute;inset:auto 20px -1px;height:2px;background:var(--heading-ink)}
.campus-tabs button:hover:not([aria-selected=true]){background:var(--heading-selected);color:var(--heading-ink)}
...
@media(max-width:700px){
  ...
  .campus-tabs button{font-size:13px;min-height:52px;padding:9px 4px;letter-spacing:.015em}
  .campus-tabs button[aria-selected=true]::after{inset-inline:10px}
...
@media(max-width:360px){
  ...
  .campus-tabs button{font-size:12px}
