  ...
  material.onBeforeCompile = shader => {
    ...
    if (projection) {
      ...
      shader.fragmentShader = shader.fragmentShader.replace('#include <lights_fragment_end>', `
        ...
        float light = (logoLight*0.80+halo*0.20)*1.35*logoOpacity + mix(filmLight,digit*1.35,digitsOnly)*countdownOpacity;
        reflectedLight.directDiffuse += projectionColor * (light + spot*0.075) * incidence * projectionStrength;
      `)
    }
  }
  material.customProgramCacheKey = () => `ivy-velvet-a-${projection ? 'projection' : 'trim'}-6`
}
...
export function createEntranceCurtain(canvas: HTMLCanvasElement, host: HTMLElement, logoUrl = '/assets/ivy-30th-anniversary-projection.png') {
  ...
  const projection: ProjectionUniforms = {
    ...
    digitOffsets: { value: new THREE.Vector3(...digitOffsets) }, digitsOnly: { value: 0 },
  ...
  function resize() {
    ...
    projection.digitsOnly.value = desktop ? 1 : 0
    ...
    const aperture = desktop ? filmHeight*0.76*2/height : Math.min(1.04,aspect*1.5)
