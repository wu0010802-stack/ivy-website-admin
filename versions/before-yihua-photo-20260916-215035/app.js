...
const campuses = {
 yihua:{name:'義華校',district:'三民區',address:'高雄市三民區義華路68號',phone:'07-392-8366',image:'campus',photoPos:'58% center',intro:'在義華路上，走進孩子的日常。',description:'從戶外廣場、綠色園藝到共創教室，認識孩子每天活動、探索與學習的空間。義華校重視愛與關懷、閱讀素養與生活自理，陪伴孩子練習與世界相處。',line:'https://lin.ee/gwl8fnA',facebook:'https://www.facebook.com/ivy.kids.fb/',fbNote:'義華校粉絲專頁'},
...
const campusStageInfo = (c,key)=>`<h3 class="campus-stage-name">${c.name}<span>高雄 · ${c.district}</span></h3><dl class="campus-stage-facts"><div><dt>${icon('map-pin')}所在地</dt><dd>${c.address}</dd></div><div><dt>${icon('phone')}參觀專線</dt><dd><a href="tel:${c.phone}">${c.phone}</a></dd></div></dl><div class="campus-stage-actions"><a class="button primary" href="${bookingLink(key)}">預約參觀${c.name}</a><a class="text-link" href="#/${key}">認識${c.name}</a></div>${campusLinks(c)}`;
// 首頁五校輪播：保留 A2 版面與單一軌道，每 6 秒切換；滑鼠停留、鍵盤操作與觸控期間暫停。
const campusShowcase = (selected='yihua')=>{
 ...
 return `<div class="campus-picker"><span class="campus-picker-nav">${prev}</span><div class="campus-track" role="tablist" aria-label="選擇校區">${segs}</div><span class="campus-picker-nav">${next}</span></div><div class="campus-stage" id="campus-stage" role="tabpanel" aria-labelledby="campus-tab-${selected}"><figure class="campus-stage-media" style="--photo-pos:${c.photoPos||'center'}">${img(c.image,c.name+'校園外觀','campus-stage-photo')}<div class="campus-stage-nav">${prev}${next}</div></figure><div class="campus-stage-side"><div class="campus-map campus-map-stage">${campusMap(selected)}<a class="map-embed-link" href="${dirURL(c)}" ${external}>${icon('navigation-arrow')}規劃路線 ${arrow}</a></div><div class="campus-stage-info">${campusStageInfo(c,selected)}</div></div></div><p class="sr-only" aria-live="off" id="campus-live"></p>`;
};
...
<section class="section campuses" id="campuses" aria-roledescription="輪播" aria-labelledby="campuses-heading"><div class="container"><div class="section-heading is-centered"><div><span class="eyebrow">Campuses</span><h2 class="section-title" id="campuses-heading">分校資訊</h2></div></div>${campusShowcase()}</div></section>
...
function setupCampusShowcase(){
 ...
 function apply(c,key){photo.src=photoSrc(c.image);photo.alt=c.name+'校園外觀';media.style.setProperty('--photo-pos',c.photoPos||'center');info.innerHTML=campusStageInfo(c,key);}
 function select(key,focusTab=false,announce=true){
  ...
  // 照片先預載、內容淡出後一起換，避免閃到舊圖；減少動態時直接換。
  const token=++switchId,next=new Image();next.src=photoSrc(c.image);
  ...
  transitionTimer=setTimeout(()=>{if(next.complete)done();else{next.addEventListener('load',done,{once:true});next.addEventListener('error',done,{once:true});}},200);
 }
 function onClick(e){
  syncPlayback();
...
function render(){const version=++routeVersion;const parts=location.hash.replace(/^#\/?/,'').split('/');let page=parts[0]||'home';if(!campuses[page]&&page!=='visit')page='home';const school=page==='visit'&&campuses[parts[1]]?parts[1]:'';const id=page==='visit'?'':parts[1]||'';const pageKey=page+(page==='visit'?'/'+school:'');closeMenu();if(currentPage!==pageKey){disposeTour();disposeMedia();disposeMedia=()=>{};disposeCampusShowcase();if(dialog.open)dialog.close();currentPage=pageKey;main.innerHTML=page==='home'?home():page==='visit'?visitPage(school):campusPage(page);document.title=page==='home'?'常春藤幼兒園｜每一天都有新發現':page==='visit'?'預約校園參觀｜常春藤幼兒園':campuses[page].name+'｜常春藤幼兒園';document.querySelector('.header-book').href=bookingLink(campuses[page]?page:'');if(page==='visit')setupBooking(school);if(page==='home'){setupDayExperience();setupVideo();setupCampusShowcase();}setupCampusTour();armMapFrames(main);}document.querySelectorAll('.nav-inner a').forEach(a=>{if(a.getAttribute('href')===location.hash)a.setAttribute('aria-current','location');else a.removeAttribute('aria-current');});requestAnimationFrame(()=>{if(version!==routeVersion)return;const target=id?document.getElementById(id):null;if(target){target.scrollIntoView({block:'start',behavior:'instant'});target.setAttribute('tabindex','-1');target.focus({preventScroll:true});}else{window.scrollTo({top:0,behavior:'instant'});main.focus({preventScroll:true});}});}
