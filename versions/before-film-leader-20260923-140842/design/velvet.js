...
function entranceTimeline(elapsedMs) {
  ...
  return { phase, countdown, opening, projection, iris, logoOpacity, leaderOpacity, flash, flare, filmFrame: Math.floor(countElapsed / (1e3 / 12)), shadowOpacity, complete };
}
...
var DIGIT_CELL = 768;
var DIGIT_FAMILY = "Ivy Countdown";
...
function createEntranceCurtain(canvas, host, logoUrl = "/assets/ivy-30th-anniversary-projection.png", options = {}) {
  ...
  let digitFace;
  const fontReady = typeof FontFace === "undefined" ? Promise.resolve() : new FontFace(DIGIT_FAMILY, `url(${digitFontUrl})`, { weight: "800" }).load().then((face) => {
    if (disposed) return;
    digitFace = face;
