...
const campuses = {
 yihua:{name:'義華校',district:'三民區',address:'高雄市三民區義華路68號',phone:'07-392-8366',image:'campus',intro:'在義華路上，走進孩子的日常。',description:'從戶外廣場、綠色園藝到共創教室，認識孩子每天活動、探索與學習的空間。義華校重視愛與關懷、閱讀素養與生活自理，陪伴孩子練習與世界相處。',line:'https://lin.ee/gwl8fnA',facebook:'https://www.facebook.com/ivy.kids.fb/',fbNote:'義華校粉絲專頁'},
 ...
 international:{name:'國際校',district:'鳥松區',address:'高雄市鳥松區球場路59號',phone:'07-370-8001',image:'international',intro:'走進鳥松，認識國際校。',description:'國際校位於高雄市鳥松區球場路。透過實際走訪了解校園環境與接送路線，並向園所確認適合孩子的課程與入學安排。',line:null,facebook:ORG_FACEBOOK,fbNote:'常春藤機構粉絲專頁（國際校粉專待補）'},
...
const img = (name,alt,cls='',eager=false,priority=false)=>`<img class="${cls}" src="assets/${name}.webp" alt="${esc(alt)}" loading="${eager?'eager':'lazy'}"${priority?' fetchpriority="high"':''} decoding="async">`;
...
const campusSocial = c=>{
 const line=c.line?`<a class="campus-social line" href="${c.line}" ${external} aria-label="${c.name} LINE 官方帳號" title="LINE 官方帳號">${icon('line')}</a>`
  :`<span class="campus-social line is-pending" role="img" aria-label="${c.name} LINE 官方帳號待提供" title="LINE 官方帳號待園方提供">${icon('line')}</span>`;
 const fb=`<a class="campus-social facebook" href="${c.facebook}" ${external} aria-label="${c.name} Facebook" title="${esc(c.fbNote)}">${icon('facebook')}</a>`;
 return line+fb;
};
const campusPills = c=>{
 const line=c.line?`<a class="campus-pill line" href="${c.line}" ${external}><span class="campus-pill-icon">${icon('line')}</span><span>加 LINE 好友<small>${c.name}官方帳號</small></span></a>`
  :`<span class="campus-pill line is-pending" title="LINE 官方帳號待園方提供"><span class="campus-pill-icon">${icon('line')}</span><span>加 LINE 好友<small>${c.name}帳號待補</small></span></span>`;
 const fb=`<a class="campus-pill facebook" href="${c.facebook}" ${external}><span class="campus-pill-icon">${icon('facebook')}</span><span>Facebook<small>${esc(c.fbNote)}</small></span></a>`;
 return line+fb;
};
...
function dayExperience(){return `<section class="section day-experience" id="life" aria-labelledby="day-heading"><div class="container"><div class="day-heading"><div><span class="eyebrow">孩子的一天</span><h2 class="section-title" id="day-heading">跟著孩子，過一天。</h2></div><p>點選一個片刻，<br>看看成長如何在日常裡發生。</p></div><div class="day-trail" role="tablist" aria-label="孩子的一天，選擇生活片刻">${dayMoments.map((m,i)=>`<button type="button" class="day-tab" role="tab" id="day-tab-${m.key}" aria-controls="day-panel" aria-selected="${i===0}" tabindex="${i===0?0:-1}" data-moment="${i}"><span class="day-dot" aria-hidden="true">${icon(m.icon)}</span><span>${m.label}</span></button>`).join('')}</div><p class="sr-only" id="day-status" aria-live="polite"></p><div id="day-panel" class="day-panel" role="tabpanel" aria-labelledby="day-tab-hello" tabindex="0"></div><div class="day-bottom"><p>日常情境提案，以義華校影像呈現；各校、各班實際作息請向園所確認。</p><a class="text-link" href="#/home/campuses">想親自看看？認識五所校園</a></div></div></section>`;}
function setupDayExperience(){
 const panel=document.querySelector('#day-panel');if(!panel)return;
 const tabs=[...document.querySelectorAll('.day-tab')];let current=0,ready=false;const visited=new Set();
 function show(index,focusTab=false){
  current=index;visited.add(index);const m=dayMoments[index];
  tabs.forEach((tab,i)=>{const active=i===index;tab.setAttribute('aria-selected',String(active));tab.tabIndex=active?0:-1;tab.classList.toggle('visited',visited.has(i));});
  panel.setAttribute('aria-labelledby',`day-tab-${m.key}`);panel.dataset.moment=m.key;
  panel.innerHTML=`<figure class="day-scene">${img(m.image,m.caption,'day-photo',true)}<div class="day-photo-shade"></div><div class="day-photo-copy"><span>今天的小小記憶</span><p>${m.word}</p></div><figcaption>${m.caption}</figcaption></figure><div class="day-story"><div class="day-chapter"><span>${m.period}</span><span class="day-counter">${String(index+1).padStart(2,'0')} <span>/ 06</span></span></div><h3>${m.title.split('\n').join('<br>')}</h3><p class="day-narrative">${m.story}</p><details class="day-parent"><summary><span class="day-parent-label">給正在了解學校的你</span><span class="day-question">${m.question}<span aria-hidden="true">＋</span></span></summary><p>${m.answer}</p></details><div class="day-controls"><span class="day-visited">已探索 ${visited.size} / 6 個片刻</span><div><button type="button" data-day-prev ${index===0?'disabled':''} aria-label="上一個生活片刻">${icon('arrow-left')}</button><button type="button" data-day-next>${index===5?'再看一次'+icon('arrow-counter-clockwise'):'下一個片刻'+icon('arrow-right')}</button></div></div></div>`;
  if(focusTab){tabs[index].focus({preventScroll:true});}
  if(ready)document.querySelector('#day-status').textContent=`第 ${index+1} 個片刻：${m.label}。已探索 ${visited.size} / 6 個片刻。`;
 }
 ...
 panel.addEventListener('click',e=>{const next=e.target.closest('[data-day-next]'),previous=e.target.closest('[data-day-prev]');if(!next&&!previous)return;show(next?(current+1)%dayMoments.length:Math.max(0,current-1));if(matchMedia('(max-width: 760px)').matches){panel.focus({preventScroll:true});panel.scrollIntoView({block:'start',behavior:'instant'});}else{const control=panel.querySelector(next?'[data-day-next]':'[data-day-prev]');if(!control.disabled)control.focus({preventScroll:true});else tabs[current].focus({preventScroll:true});}});
 show(0);ready=true;
}

function home(){return `
<section class="studio-hero" aria-labelledby="home-title"><div class="container studio-hero-grid">
 <div class="studio-hero-copy"><span class="eyebrow">常春藤幼兒園 · 高雄五校</span><h1 id="home-title">在常春藤<span class="punct">，</span><br><span class="growing-word">每一天都有新發現<svg aria-hidden="true" viewBox="0 0 360 18"><path pathLength="1" d="M4 12Q100 0 183 9T354 7"/></svg></span><span class="punct">。</span></h1><p>我們將遊戲、閱讀、藝術創作與自然探索融入日常，陪孩子動手嘗試、自在表達，在每一次發現中，感受學習的驚喜與樂趣。</p><div class="studio-actions"><a class="button ghost" href="#/home/life">看看孩子的一天</a></div></div>
 ...
 </div><div class="container studio-media"><div id="video-controls" hidden><button type="button" id="video-play" aria-controls="hero-video">播放影片</button></div></div><div class="container studio-hero-foot"><span>五所校園：三民 · 左營 · 鳥松 · 仁武</span><a href="#/home/about">往下，認識我們 ${icon('arrow-down')}</a></div></section>
<section class="section studio-about" id="about"><div class="container"><div class="studio-intro"><div class="studio-intro-copy"><span class="eyebrow">關於常春藤</span><h2 class="section-title">在地近三十年，<br>把每個孩子，放在心上。</h2><p class="section-copy">在高雄深耕近三十年，常春藤以用心陪伴累積家長的信任。歐式城堡裡，有一起讀的故事、自在探索的日常，也有老師溫柔的守候。</p><p class="section-copy">我們相信，教育是「生命影響生命」。用愛與關懷理解每個孩子的不同，在快樂、健康的環境裡，陪孩子依自己的步調成長，帶著自信與好奇，走向更大的世界。</p><a class="text-link" href="#/home/campuses">認識五所校園</a></div><figure class="studio-intro-photo">${img('about-curious','常春藤的孩子在教室裡指著剛發現的東西')}<figcaption>常春藤的孩子 · 校園片刻</figcaption></figure></div>
 <div class="studio-values" id="learning"><article><span class="value-index">01 / 關懷</span><h3>每個孩子，都值得被懂。</h3><p>尊重每個孩子的步調，耐心傾聽、溫柔引導。在被理解與關愛的日常裡，也學著關心身邊的人。</p></article><article><span class="value-index">02 / 閱讀</span><h3>從一本書，打開好奇心。</h3><p>一起讀故事、聊想法，把書裡的好奇帶進生活。鼓勵孩子提問、探索，讓喜歡學習成為一生的習慣。</p></article><article><span class="value-index">03 / 自理</span><h3>自己試試，也是一種長大。</h3><p>從穿鞋、收拾到照顧自己，留一點時間讓孩子動手。在一次次「我會了」之中，累積獨立的能力與自信。</p></article></div></div></section>
${dayExperience()}
<section class="section studio-discover" id="news"><div class="container"><div class="section-heading"><div><span class="eyebrow">孩子眼中的世界</span><h2 class="section-title">把平凡的小事，<br>看成大大的發現。</h2></div><p>靠近一片葉子，走進一個故事。<br>探索，就從身邊開始。</p></div><div class="discovery-grid"><figure class="discovery-nature">${img('garden','義華校園藝照片：枝葉間的果實')}<figcaption><span>自然裡的小發現</span><p>一片綠，藏著好多為什麼。</p><small>義華校 · 綠色園藝</small></figcaption></figure><figure class="discovery-learning">${img('learning','義華校學習活動照片')}<figcaption><span>學習中的每一次嘗試</span><p>看一看，試一試，再想一想。</p><small>義華校 · 學習情境</small></figcaption></figure><div class="discovery-note"><span class="paper-circle" aria-hidden="true"></span><p>留一點空間，<br>給還沒想到的<br><strong>可能。</strong></p><a class="text-link" href="#/yihua/environment">走進義華校探索照片</a></div></div></div></section>
<section class="section campuses" id="campuses"><div class="container"><div class="section-heading"><div><span class="eyebrow">五校介紹</span><h2 class="section-title">五所校園，等你來認識。</h2></div><p>從生活圈與接送路線出發，<br>找到適合你與孩子的常春藤。</p></div><div class="campus-grid">${Object.entries(campuses).map(([key,c],i)=>`<article class="campus-card" data-campus="${key}" style="--campus-order:${i*2}"><a class="campus-picture" href="#/${key}" aria-label="認識${c.name}">${img(c.image,c.name+'官方校園照片')}<span class="campus-region">高雄 · ${c.district}</span></a><div class="campus-topline"><h3><a href="#/${key}">${c.name}</a></h3></div><p class="campus-address">${c.address}</p><div class="campus-meta"><a href="tel:${c.phone}">${icon('phone')}${c.phone}</a><a href="#/${key}">認識校區</a></div><div class="campus-links">${campusSocial(c)}<button type="button" class="campus-social map" aria-expanded="false" aria-controls="campus-map-drawer" aria-label="顯示${c.name}地圖" title="地圖與路線">${icon('map-pin')}</button></div></article>`).join('')}<div class="campus-drawer" id="campus-map-drawer" hidden></div></div></div></section>
<section class="section" id="faq"><div class="container faq-grid"><div><span class="eyebrow">參觀前的問題</span><h2 class="section-title">參觀前，<br>你可能想知道。</h2><p class="section-copy">每個家庭都有不同的考量。<br>把問題帶來，我們一起聊聊。</p></div>${faq()}</div></section>${banner()}`;}
...
<section class="section campuses" id="contact"><div class="container detail-grid"><div class="contact-location"><span class="eyebrow">交通與聯絡</span><h2 class="section-title">我們在這裡，等你來。</h2><a class="phone-link" href="tel:${c.phone}">${icon('phone')}${c.phone}</a><p>${c.address}</p><div class="campus-pills">${campusPills(c)}</div><div><a class="button primary" href="${bookingLink(key)}">預約${c.name}</a></div></div>${campusMapFrame(c,'campus-contact-map')}</div></section>${banner(key)}`;}
...
function setupVideo(){
 ...
 if(!video)return;
 let disposed=false,resumeOnVisible=false;
 const sync=()=>{play.innerHTML=video.paused?icon('play')+'播放影片':icon('pause')+'暫停影片';};
 const start=()=>video.play().catch(()=>{if(!disposed)sync();});
 video.muted=true;
 video.addEventListener('play',sync);
 video.addEventListener('pause',sync);
 video.addEventListener('error',()=>{
  if(disposed)return;
  video.hidden=true;controls.hidden=true;
 });
 ...
 if(!matchMedia('(prefers-reduced-motion: reduce)').matches&&!frugal)start();
 const visibility=()=>{
...
let disposeCampusCards=()=>{};
function setupCampusCards(){
 const grid=document.querySelector('.campus-grid');if(!grid)return;
 const drawer=document.querySelector('#campus-map-drawer');
 const keys=Object.keys(campuses);
 const mq=matchMedia('(max-width:760px)');
 let open=mq.matches?null:keys[0];
 function paint(){
  grid.querySelectorAll('.campus-card').forEach(card=>{
   card.querySelector('.campus-social.map').setAttribute('aria-expanded',String(card.dataset.campus===open));
  });
  if(!open){drawer.hidden=true;drawer.innerHTML='';return;}
  const c=campuses[open];const i=keys.indexOf(open);
  // 桌機／平板固定放在整排最後（order:99，見 styles.css），避免 grid-column:1/-1 卡在列中間打斷其他卡片換行；
  // 只有手機單欄版才需要照順序插到被點的卡片後面。
  drawer.style.order=mq.matches?String(i*2+1):'';
  drawer.innerHTML=`<div class="campus-drawer-info"><header><h3>${c.name}<span class="campus-region-label">高雄 · ${c.district}</span></h3><button type="button" class="campus-drawer-close" aria-label="收合${c.name}地圖">${icon('x')}</button></header><dl><div><dt>${icon('map-pin')}所在地</dt><dd>${c.address}</dd></div><div><dt>${icon('phone')}參觀專線</dt><dd><a href="tel:${c.phone}">${c.phone}</a></dd></div></dl><div class="campus-pills">${campusPills(c)}</div><div class="campus-drawer-cta"><a class="button yellow" href="${bookingLink(open)}">${icon('calendar-check')}預約參觀${c.name}</a><a class="text-link" href="#/${open}">認識校區</a></div></div>${campusMapFrame(c,'campus-drawer-map')}`;
  drawer.hidden=false;
  armMapFrames(drawer);
  drawer.querySelector('.campus-drawer-close').addEventListener('click',()=>{
   const btn=grid.querySelector(`.campus-card[data-campus="${open}"] .campus-social.map`);
   open=null;paint();btn&&btn.focus({preventScroll:true});
  });
 }
 function onClick(e){
  const btn=e.target.closest('.campus-social.map');if(!btn)return;
  const key=btn.closest('.campus-card').dataset.campus;
  const wasOpen=open===key;
  open=wasOpen?null:key;paint();
  if(!wasOpen&&mq.matches)requestAnimationFrame(()=>drawer.scrollIntoView({block:'nearest',behavior:matchMedia('(prefers-reduced-motion:no-preference)').matches?'smooth':'auto'}));
 }
 grid.addEventListener('click',onClick);
 paint();
 disposeCampusCards=()=>{grid.removeEventListener('click',onClick);disposeCampusCards=()=>{};};
}
...
function render(){const version=++routeVersion;const parts=location.hash.replace(/^#\/?/,'').split('/');let page=parts[0]||'home';if(!campuses[page]&&page!=='visit')page='home';const school=page==='visit'&&campuses[parts[1]]?parts[1]:'';const id=page==='visit'?'':parts[1]||'';const pageKey=page+(page==='visit'?'/'+school:'');closeMenu();if(currentPage!==pageKey){disposeTour();disposeMedia();disposeMedia=()=>{};disposeCampusCards();if(dialog.open)dialog.close();currentPage=pageKey;main.innerHTML=page==='home'?home():page==='visit'?visitPage(school):campusPage(page);document.title=page==='home'?'常春藤幼兒園｜每一天都有新發現':page==='visit'?'預約校園參觀｜常春藤幼兒園':campuses[page].name+'｜常春藤幼兒園';document.querySelector('.header-book').href=bookingLink(campuses[page]?page:'');if(page==='visit')setupBooking(school);if(page==='home'){setupDayExperience();setupVideo();setupCampusCards();}setupCampusTour();armMapFrames(main);}document.querySelectorAll('.nav-inner a').forEach(a=>{if(a.getAttribute('href')===location.hash)a.setAttribute('aria-current','location');else a.removeAttribute('aria-current');});requestAnimationFrame(()=>{if(version!==routeVersion)return;const target=id?document.getElementById(id):null;if(target){target.scrollIntoView({block:'start',behavior:'instant'});target.setAttribute('tabindex','-1');target.focus({preventScroll:true});}else{window.scrollTo({top:0,behavior:'instant'});main.focus({preventScroll:true});}});}
