...
var projectionLighting = (
  ...
  float gateOpen = 0.49*leaderIris;
  float lit = 1.0-smoothstep(gateOpen-gateAa-focus*0.4,gateOpen+gateAa,gateRadius);
  ...
  float emulsion = max(max(ringOuter,ringInner),max(max(crossLines*0.9,hand*0.85),glyph));
  // Worn print: a few dust specks per frame and an occasional running scratch.
  float dust = 0.0;
  for (int i = 0; i < 3; i++) {
    float speckSeed = filmFrame*7.1 + float(i)*13.7;
    vec2 speck = vec2(pileHash(speckSeed),pileHash(speckSeed+41.3))*0.8-0.4;
    float speckSize = 0.003+0.006*pileHash(speckSeed+5.9);
  ...
  // A lamp hotspot falls off toward the gate edge, as an old projector does.
  float tone = mix(1.0,0.55,swept)*(0.6+0.4*(1.0-pow(min(1.0,gateRadius/0.49),2.0)));
  float flicker = 0.95+0.05*pileHash(filmFrame*1.37+3.0);
  ...
  float leaderLight = lit*(tone*(1.0-0.92*emulsion)*(1.0-0.75*dust)*flicker*(0.985+grain*0.03) + scratch*0.22)
  ...
  vec3 leaderColour = (diffuseColor.rgb*6.0 + rimColor*rim*0.35 + 0.2)*projectionColor*leaderLight;
  reflectedLight.directDiffuse += (slideLight + poolLight + leaderColour) * incidence;
`
);
function velvetUniforms() {
...
${lights}`);
  ...
  material.customProgramCacheKey = () => `ivy-velvet-a-${key}-15`;
}
...
function createEntranceCurtain(canvas, host, logoUrl = "/assets/ivy-30th-anniversary-projection.png", options = {}) {
  ...
  deformMaterial(pelmetCloth, valanceSurface, valanceUniforms, { stage: { ...velvetUniforms(), seed: "7.0", flies: 0.25 } });
  const pelmetDepth = new THREE.MeshDepthMaterial({ depthPacking: THREE.RGBADepthPacking, side: THREE.DoubleSide });
  deformMaterial(pelmetDepth, valanceSurface, valanceUniforms);
  const pelmet = new THREE.Mesh(drape, pelmetCloth);
  pelmet.frustumCulled = false;
  pelmet.castShadow = pelmet.receiveShadow = true;
  pelmet.customDepthMaterial = pelmetDepth;
  scene.add(pelmet);
  const cordTrim = trimMaterial();
  deformMaterial(cordTrim, valanceSurface, valanceUniforms, { braid: valanceBraid, stage: { ...velvetUniforms(), seed: "9.0", flies: 0 } });
  const cord = new THREE.Mesh(edge, cordTrim);
  cord.frustumCulled = false;
  scene.add(cord);
  materials.push(pelmetCloth, pelmetDepth, cordTrim);
