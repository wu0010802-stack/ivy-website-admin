...
let timeline, duration = 6400, engine, frame = 0, progress = 0, last = 0, playing = false, failed = false;
const reduced = matchMedia('(prefers-reduced-motion: reduce)');
function draw() {
  engine?.draw(progress);
  $('#progress').value = Math.round(progress * 1000);
  $('#time').value = `${(progress * duration / 1000).toFixed(1)} / 6.4 秒`;
  const state = timeline?.(progress * duration);
  $('#stage').dataset.countdown = state?.countdown ?? 3;
  $('#stage').dataset.opening = state?.opening ?? 0;
  $('#countdown-status').textContent = state?.complete ? '開幕完成' : state?.countdown ? `開幕倒數 ${state.countdown}` : '布幕開啟中';
