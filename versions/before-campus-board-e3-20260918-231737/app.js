...
// 預覽用：index.html?campus=a|b|c#/home 把「分校資訊」改成滿版釘住、往下捲切換五校的三個方向（a 全幅顯影／b 疊相片／c 地圖引路），樣式在 design/campus-scroll-directions/campus-scroll.css。
const CAMPUS_SCROLL=(()=>{const v=new URLSearchParams(location.search).get('campus');return ['a','b','c','a1','a2','a3','a1b','a1c','d','d1','d2','d3','e1','e2','e3'].includes(v)?v:'';})();// e1 窄卡／e2 全出血／e3 浮卡跨色帶：卡片相對米白背景的三種比例（底板同 d1）// d1 寬幅細板／d2 標準大板／d3 高照片窄板：底板卡的三種比例// d 底板卡：不滿版，站上原框架（置中標題、右上線稿、校名軌道）＋ a1c 的照片＋墨綠底板// a1 藍圖板／a2 米白資訊帶／a3 素描顯影：A 保留建築線稿的三種放法；a1b／a1c 是選定 a1 後的版面變化（Google 地圖、小線稿、不留「認識Ｘ校」）
if(CAMPUS_SCROLL){document.documentElement.dataset.campusScroll=CAMPUS_SCROLL;const l=document.createElement('link');l.rel='stylesheet';l.href='design/campus-scroll-directions/campus-scroll.css';document.head.append(l);}
...
${/^[de]/.test(CAMPUS_SCROLL)?campusBoardSection(CAMPUS_SCROLL):CAMPUS_SCROLL?campusScrollSection(CAMPUS_SCROLL):`<section class="section campuses campus-panorama" id="campuses" aria-roledescription="輪播" aria-labelledby="campuses-heading">${campusArtwork('yihua')}<div class="container"><div class="section-heading is-centered"><div><span class="eyebrow">Campuses</span><h2 class="section-title" id="campuses-heading">分校資訊</h2></div></div>${campusShowcase()}</div></section>`}</div>
</div></div>
...
function setupCampusShowcase(){
 ...
 const board=root.dataset.variant==='d';
 function apply(c,key){photo.src=photoSrc(c.image);photo.alt=c.name+'校園外觀';media.style.setProperty('--photo-pos',c.photoPos||'center');media.style.setProperty('--panorama-pos',c.panoramaPos||'center 55%');info.innerHTML=board?boardInfo(c):campusStageInfo(c);root.querySelector('.campus-stage-actions').innerHTML=board?boardActions(c,key):campusStageActions(c,key);root.querySelector('.campus-art-building').src=campusArtSrc(key);}
 function select(key,focusTab=false,announce=true){
  ...
  if(svg){svg.querySelectorAll('.campus-map-pin').forEach(p=>{const on=p.dataset.campus===key;p.classList.toggle('is-selected',on);if(on)svg.append(p);});svg.setAttribute('aria-label','五校位置示意圖，目前標示'+c.name);}
  if(route)route.href=dirURL(c);
...
// ?campus=a|b|c：分校資訊改成滿版釘住的舞台，五校靠往下捲切換；三個方向共用同一份 DOM，差別全在 CSS 與 --t／--d 的用法。
function campusScrollSection(v){
 const keys=Object.keys(campuses);
 const a1=/^a1/.test(v);// a1 系列：Google 地圖嵌在每校面板裡、不留「認識Ｘ校」
 const panels=keys.map((key,i)=>{const c=campuses[key];return `<article class="cs-panel${i===0?' is-active':''}" data-campus="${key}" style="--i:${i};--d:${i};--photo-pos:${c.photoPos||'center'};--panorama-pos:${c.panoramaPos||'center 55%'}"><figure class="cs-photo">${img(c.image,c.name+'校園外觀','cs-img',i<2)}</figure>${/^a\d/.test(v)?`<img class="cs-art" src="${campusArtSrc(key)}" alt="" loading="lazy" decoding="async">`:''}<div class="cs-body"><h3 class="cs-name"><span class="cs-name-zh">${c.name}</span><span class="cs-name-area">高雄 · ${c.district}</span></h3><p class="cs-intro">${c.intro}</p><dl class="cs-facts"><div><dt>${icon('map-pin')}所在地</dt><dd>${c.address}</dd></div><div><dt>${icon('phone')}參觀專線</dt><dd><a href="tel:${c.phone}">${c.phone}</a></dd></div></dl><div class="cs-actions">${a1?`<a class="button primary" href="${bookingLink(key)}">預約參觀${c.name}${icon('arrow-right')}</a><a class="text-link cs-maplink" href="${mapURL(c)}" ${external}>在 Google 地圖開啟${arrow}</a>`:campusStageActions(c,key)}</div>${campusLinks(c)}</div>${a1?campusMapFrame(c,'cs-gmap'):''}</article>`;}).join('');
 const rail=keys.map((key,i)=>`<button type="button" class="cs-rail-item" data-campus="${key}" aria-pressed="${i===0}"><span class="cs-rail-num">0${i+1}</span><span class="cs-rail-name">${campuses[key].name}</span><span class="cs-rail-area">${campuses[key].district}</span></button>`).join('');
 const vb=v==='c'?'80 150 480 270':'100 200 460 215';
 return `<section class="section campuses campus-panorama campus-scroll ${[1,2,3].map(n=>v.length>=n?'cs-'+v.slice(0,n):'').join(' ')}" id="campuses" data-variant="${v}" aria-labelledby="campuses-heading" style="--n:${keys.length};--t:0"><div class="cs-stage">${v==='a'?campusArtwork(keys[0]):''}<header class="cs-head"><span class="eyebrow">Campuses</span><h2 class="section-title" id="campuses-heading">分校資訊</h2><p class="cs-hint">${icon('arrow-down')}往下滑，走過五所校園</p></header><nav class="cs-rail" aria-label="選擇校區">${rail}</nav><p class="cs-counter" aria-hidden="true"><b class="cs-counter-now">01</b><span>/ 0${keys.length}</span></p><div class="cs-map">${campusMap(keys[0],vb)}<a class="map-embed-link cs-route" href="${dirURL(campuses[keys[0]])}" ${external}>${icon('navigation-arrow')}規劃路線 ${arrow}</a></div><div class="cs-panels">${panels}</div></div><p class="sr-only" aria-live="polite" id="campus-live"></p></section>`;
}
// ?campus=d 底板卡：外框沿用站上的置中標題、右上線稿與校名軌道；舞台改成「全寬照片＋墨綠底板」，不滿版釘住、不放地圖，切換走 setupCampusShowcase()。
const boardInfo=c=>`<h3 class="cs-name"><span class="cs-name-zh">${c.name}</span><span class="cs-name-area">高雄 · ${c.district}</span></h3><dl class="cs-facts"><div><dt>${icon('map-pin')}所在地</dt><dd>${c.address}</dd></div><div><dt>${icon('phone')}參觀專線</dt><dd><a href="tel:${c.phone}">${c.phone}</a></dd></div></dl>${campusLinks(c)}`;
...
function campusBoardSection(v='d'){
 const keys=Object.keys(campuses),selected=keys[0],c=campuses[selected];
 const segs=keys.map(key=>{const on=key===selected;return `<button type="button" role="tab" class="campus-seg" id="campus-tab-${key}" aria-selected="${on}" tabindex="${on?0:-1}" aria-controls="campus-stage" data-campus="${key}">${campuses[key].name}</button>`;}).join('');
 const prev=`<button type="button" class="campus-stage-btn" data-step="-1" aria-label="上一校">${icon('arrow-left')}</button>`,next=`<button type="button" class="campus-stage-btn" data-step="1" aria-label="下一校">${icon('arrow-right')}</button>`;
 return `<section class="section campuses campus-panorama campus-board board-${v[0]==='e'?'d1 board-'+v:v}" id="campuses" data-variant="d" aria-roledescription="輪播" aria-labelledby="campuses-heading">${campusArtwork(selected)}<div class="container"><div class="section-heading is-centered"><div><span class="eyebrow">Campuses</span><h2 class="section-title" id="campuses-heading">分校資訊</h2></div></div><div class="campus-picker"><span class="campus-picker-nav">${prev}</span><div class="campus-track" role="tablist" aria-label="選擇校區">${segs}</div><span class="campus-picker-nav">${next}</span></div><div class="campus-stage board-stage" id="campus-stage" role="tabpanel" aria-labelledby="campus-tab-${selected}"><figure class="campus-stage-media" style="--photo-pos:${c.photoPos||'center'};--panorama-pos:${c.panoramaPos||'center 55%'}">${img(c.image,c.name+'校園外觀','campus-stage-photo')}</figure><div class="campus-stage-nav">${prev}${next}</div><div class="board-plate"><div class="campus-stage-info">${boardInfo(c)}</div><div class="campus-stage-actions">${boardActions(c,selected)}</div></div></div></div><p class="sr-only" aria-live="off" id="campus-live"></p></section>`;
}
let disposeCampusScroll=()=>{};
function setupCampusScroll(){
 const root=document.querySelector('.campus-scroll');if(!root)return;
 const stage=root.querySelector('.cs-stage'),panels=[...root.querySelectorAll('.cs-panel')],rail=[...root.querySelectorAll('.cs-rail-item')],keys=panels.map(p=>p.dataset.campus);
 const svg=root.querySelector('.campus-map-svg'),route=root.querySelector('.cs-route'),counter=root.querySelector('.cs-counter-now'),live=root.querySelector('#campus-live');
 const reduce=matchMedia('(prefers-reduced-motion: reduce)');
 const clamp=(n,lo=0,hi=1)=>Math.max(lo,Math.min(hi,n));
 const n=keys.length;
 let disposed=false,frame=0,start=0,distance=1,screen=1,active=-1,pan=null,liveTimer=0;
 // c 方向：把地圖內容包進一層 <g>，往下捲時鏡頭平移到目前校區；圖釘間補一條虛線路徑。
 if(root.dataset.variant==='c'&&svg){
  const pins=keys.map(k=>CAMPUS_MAP.pins[k]);
  const pathD=pins.map((p,i)=>(i?'L':'M')+p.x+' '+p.y).join(' ');
  svg.innerHTML=`<g class="cs-pan">${svg.innerHTML}<path class="cs-route-line" d="${pathD}" pathLength="1"/></g>`;
  const g=svg.querySelector('.cs-pan'),line=g.querySelector('.cs-route-line');
  g.insertBefore(line,g.querySelector('.campus-map-pin'));// 路徑畫在區塊之上、圖釘之下
  pan={g,pins,line};
 }
 // Google 地圖 embed 載入時會搶焦點把頁面捲走，所以只在區塊接近視窗後才載入，且一次只載當前校。
 let armed=false;
 function loadMap(){if(!armed||active<0)return;const f=panels[active].querySelector('iframe[data-src]:not([src])');if(f)f.src=f.dataset.src;}
 const armer=new IntersectionObserver(entries=>{if(entries.some(e=>e.isIntersecting)){armed=true;loadMap();armer.disconnect();}},{rootMargin:'160px 0px'});
 armer.observe(stage);
 function setActive(i,announce){
  if(i===active)return;active=i;const key=keys[i],c=campuses[key];loadMap();
  panels.forEach((p,j)=>{const on=j===i;p.classList.toggle('is-active',on);p.inert=!on;});
  rail.forEach((b,j)=>b.setAttribute('aria-pressed',String(j===i)));
  counter.textContent='0'+(i+1);
  if(svg){svg.querySelectorAll('.campus-map-pin').forEach(p=>{const on=p.dataset.campus===key;p.classList.toggle('is-selected',on);if(on)p.parentNode.append(p);});svg.setAttribute('aria-label','五校位置示意圖，目前標示'+c.name);}
  if(route)route.href=dirURL(c);
  // 建築線稿跟著換：先淡出、預載新圖再換上淡入。
  const art=root.querySelector('.campus-art-building');
  if(art){const src=campusArtSrc(key);art.classList.add('is-swapping');const next=new Image();next.src=src;next.decode().catch(()=>{}).then(()=>{if(disposed||art.dataset.want!==src)return;art.src=src;requestAnimationFrame(()=>art.classList.remove('is-swapping'));});art.dataset.want=src;}
  clearTimeout(liveTimer);if(announce)liveTimer=setTimeout(()=>{if(!disposed)live.textContent=`目前顯示 ${c.name}，高雄${c.district}`;},400);
 }
 function update(){
  frame=0;if(disposed)return;
  const p=clamp((scrollY-start)/distance),t=p*(n-1);
  root.style.setProperty('--t',t.toFixed(4));
  root.style.setProperty('--p',p.toFixed(4));
  panels.forEach((el,i)=>el.style.setProperty('--d',(i-t).toFixed(4)));
  root.classList.toggle('is-pinned',t>.2);
  if(pan){
   const i=Math.floor(t),f=t-i,a=pan.pins[i],b=pan.pins[Math.min(n-1,i+1)];
   const x=a.x+(b.x-a.x)*f,y=a.y+(b.y-a.y)*f;
   const phone=innerWidth<=760,vb=svg.viewBox.baseVal,cx=vb.x+vb.width*.42,cy=vb.y+vb.height*(phone?.5:.52),s=phone?1:1.25;// 圖釘停在偏左，右邊留給校名牌
   pan.g.setAttribute('transform',`translate(${cx} ${cy}) scale(${s}) translate(${(-x).toFixed(1)} ${(-y).toFixed(1)})`);
   pan.line.style.strokeDashoffset=String(1-p);
  }
  setActive(Math.round(t),active>=0);
 }
 const schedule=()=>{if(!disposed&&!frame)frame=requestAnimationFrame(update);};
 function measure(){
  screen=document.documentElement.clientHeight;
  const r=root.getBoundingClientRect();start=r.top+scrollY;distance=Math.max(1,r.height-screen);
  update();
 }
 function go(i){const y=start+distance*(i/(n-1));scrollTo({top:Math.round(y)+1,behavior:reduce.matches?'instant':'smooth'});}
 const onClick=e=>{const b=e.target.closest('.cs-rail-item');if(b){go(keys.indexOf(b.dataset.campus));return;}const pin=e.target.closest('.campus-map-pin');if(pin)go(keys.indexOf(pin.dataset.campus));};
 const onKey=e=>{const b=e.target.closest('.cs-rail-item');if(!b)return;const i=keys.indexOf(b.dataset.campus);const next={ArrowDown:i+1,ArrowRight:i+1,ArrowUp:i-1,ArrowLeft:i-1,Home:0,End:n-1}[e.key];if(next===undefined)return;e.preventDefault();const j=clamp(next,0,n-1);rail[j].focus({preventScroll:true});go(j);};
 root.addEventListener('click',onClick);root.addEventListener('keydown',onKey);
 addEventListener('scroll',schedule,{passive:true});addEventListener('resize',measure,{passive:true});
 const bodyObserver=new ResizeObserver(measure);bodyObserver.observe(document.body);
 document.fonts.ready.then(()=>{if(!disposed)measure();});
 measure();
 disposeCampusScroll=()=>{disposed=true;cancelAnimationFrame(frame);clearTimeout(liveTimer);bodyObserver.disconnect();armer.disconnect();root.removeEventListener('click',onClick);root.removeEventListener('keydown',onKey);removeEventListener('scroll',schedule);removeEventListener('resize',measure);disposeCampusScroll=()=>{};};
}
...
function render(){const version=++routeVersion;const parts=location.hash.replace(/^#\/?/,'').split('/');let page=parts[0]||'home';if(!campuses[page]&&page!=='visit')page='home';const school=page==='visit'&&campuses[parts[1]]?parts[1]:'';const id=page==='visit'?'':parts[1]||'';const pageKey=page+(page==='visit'?'/'+school:'');closeMenu();homepageNews.close();if(currentPage!==pageKey){disposeTour();disposeHomeReveal();disposeDay();disposeBeliefCurtain();disposeDayCurtain();disposeMedia();disposeMedia=()=>{};disposeCampusShowcase();disposeCampusScroll();if(dialog.open)dialog.close();currentPage=pageKey;main.innerHTML=page==='home'?home():page==='visit'?visitPage(school):campusPage(page);document.title=page==='home'?'常春藤幼兒園｜每一天都有新發現':page==='visit'?'預約校園參觀｜常春藤幼兒園':campuses[page].name+'｜常春藤幼兒園';{const book=document.querySelector('.header-book');book.href=bookingLink(campuses[page]?page:'');// 人已經在預約頁時，這顆指向原地；視覺照舊，但要讓輔助科技知道。
if(page==='visit')book.setAttribute('aria-current','page');else book.removeAttribute('aria-current');}if(page==='visit')setupBooking(school);if(page==='home'){setupDayExperience();setupBeliefCurtain();setupDayCurtain();setupRelayStudy();setupHomeReveal(setupVideo());if(CAMPUS_SCROLL&&!/^[de]/.test(CAMPUS_SCROLL))setupCampusScroll();else setupCampusShowcase();}setupCampusTour();armMapFrames(main);setupHeaderMode(page);}document.querySelectorAll('.nav-inner a').forEach(a=>{if(a.getAttribute('href')===location.hash)a.setAttribute('aria-current','location');else a.removeAttribute('aria-current');});requestAnimationFrame(()=>{if(version!==routeVersion)return;const target=id?document.getElementById(id):null;if(target){target.scrollIntoView({block:'start',behavior:'instant'});target.setAttribute('tabindex','-1');target.focus({preventScroll:true});}else{window.scrollTo({top:0,behavior:'instant'});main.focus({preventScroll:true});}});}
homepageNews.init();
