...
vec4 logoTransmission(vec2 uv) {
  ...
  // Remove only the nearly white paper, keeping the cream faces and every
  // original brand colour. Decode the source sRGB before adding projected light.
  float pigment = 1.0-min(ink.r,min(ink.g,ink.b));
  float mask = smoothstep(0.045,0.105,pigment)*insideProjection(uv);
  vec3 linearInk = mix(ink/12.92,pow((ink+0.055)/1.055,vec3(2.4)),step(vec3(0.04045),ink));
  return vec4(linearInk*mask,mask);
}
`
);
function deformMaterial(material, uniforms, projection) {
  material.onBeforeCompile = (shader) => {
    ...
    if (projection) {
      ...
      shader.fragmentShader = shader.fragmentShader.replace("#include <lights_fragment_end>", `
        ...
        vec4 halo = (logoTransmission(logoUv+vec2(0.0025,0.0))
        ...
        // Reject the source paper's antialiased fringe while preserving the
        // enclosed cream faces. This keeps the projection from gaining a rim.
        float cleanEdge = smoothstep(0.55,0.95,halo.a);
        logoLight *= cleanEdge;
        halo *= cleanEdge;
        ...
        float clothUnderLogo = 1.0-crestCoverage*0.92;
        reflectedLight.directDiffuse *= clothUnderLogo;
        reflectedLight.indirectDiffuse *= clothUnderLogo;
        vec3 crestLight = (logoLight.rgb*0.90+halo.rgb*0.10)*logoOpacity;
        float light = mix(filmLight,cinemaLight,cinemaProjection)*countdownOpacity;
  ...
  material.customProgramCacheKey = () => `ivy-velvet-a-${projection ? "projection" : "trim"}-8`;
}
