...
日期：2026-09-19。修訂：v2，使用者已選定 Nuxt 前台；本規格已更新，產品程式尚未實作。
...
- 編輯不直接影響官網。每次保存形成 revision，支援草稿、待審核、已核准、發布及退回原因。
- 總管理者／授權分校管理者可發布自己權限範圍內容；內容編輯只能送審。
...
- 排程發布綁定明確 revision 與時區；執行時重新驗證權限、素材就緒及分校狀態。
