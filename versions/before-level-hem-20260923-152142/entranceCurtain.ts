...
const velvetPalette = {
  fabric: '#561326', trim: '#bf9a52',
  sky: '#f4d8ca', ground: '#1e0a14', key: '#fff0dd', fill: '#eab6bc',
  projection: '#efce92', footlight: '#ffb778', clothTint: '#ff8a99', whiteLamp: '#fff3e4'
}

// Comparison: the accepted rose sheen against a deeper crimson one.
const sheenTones = {
  rose: { sheen: '#c46f7e', rim: '#d98794' },
  crimson: { sheen: '#a83a48', rim: '#cf4a58' }
}
type SheenTone = keyof typeof sheenTones

type EntranceCurtainOptions = {
  /** Leader numerals face; the fallback sans stays drawn until it loads. */
  digitFontUrl?: string
  /** Comparison only (`?sheen=crimson`): velvet highlight tone. */
  sheenTone?: SheenTone
  /** Comparison only (`?lamp=white`): the leader's lamp colour. */
  leaderLamp?: 'warm' | 'white'
}
...
function velvetUniforms(pileDensity: { value: number }, tone: SheenTone): StageUniforms {
...
function velvetMaterial(tone: SheenTone) {
  return new THREE.MeshPhysicalMaterial({
    color: velvetPalette.fabric, roughness: 0.96, metalness: 0,
    sheen: 1, sheenColor: sheenTones[tone].sheen, sheenRoughness: 0.55,
...
// A gold tassel turned from a profile: knob, bound neck, then a flared skirt.
// The origin is the hanging point, so rotating it swings like a pendulum.
function tasselGeometry() {
  const profile = [[0.003,0],[0.011,-0.004],[0.015,-0.013],[0.011,-0.022],[0.006,-0.028],[0.009,-0.033],
    [0.006,-0.038],[0.012,-0.045],[0.016,-0.075],[0.02,-0.105],[0.022,-0.122],[0,-0.124]]
...
export function createEntranceCurtain(canvas: HTMLCanvasElement, host: HTMLElement, logoUrl = '/assets/ivy-30th-anniversary-projection.png', options: EntranceCurtainOptions = {}) {
  ...
  for (const side of [-1, 1]) {
    ...
    deformMaterial(trim, drapeSurface, uniforms, { braid: hemBraid, stage: { ...velvetUniforms(drapePile, sheenTone), seed: '5.0', flies: 0, foot: 0.35 }, lift: 0.003 })
  ...
  deformMaterial(cordTrim, valanceSurface, valanceUniforms, { braid: valanceBraid, stage: { ...velvetUniforms(valancePile, sheenTone), seed: '9.0', flies: 0 }, lift: 0.003 })
