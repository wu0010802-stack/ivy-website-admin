...
- 大幅影片封面、品牌介紹、五校總覽、教育理念、校園活動、FAQ。
...
使用者在畫布 https://claude.ai/artifact/8PpG9rz5rc5UV1VaHfoGas 第三頁選定探索版 A3，並指定「五校放同一條長橢圓軌道、照片多些圓角」；已照定案改進首頁，取代中午上站的五校目錄版。

- 分區選校軌道：地區小標對齊各組，五校在同一條淡灰米底的長橢圓軌道裡，選中的是實心綠膠囊；組與組之間一條細線。
- 左大照片（4:3、24px 圓角、逐校裁切）＋校區介紹句＋上一／下一校；右欄校名地區、地址電話、「預約參觀Ｘ校」主鈕與「認識Ｘ校」校區頁入口、LINE／Facebook 徽章、210px 小示意地圖（16px 圓角）。
- 切換：軌道 tab 鍵盤操作、上一／下一校首尾循環、點地圖圖釘；照片預載後 200ms 淡入淡出，減少動態直接換，aria-live 播報目前校區，不自動輪播。
- 打包器改為替換 `photoSrc` 單一定義；`preview.html` 已重打包，照片、字型皆內嵌。
- 驗證：1440px 與 390px 無水平溢出、無主控台錯誤；tab／方向鍵／上一下一校／圖釘切換後照片、介紹、資訊、地圖、路線、播報全部同步。改前快照 `versions/before-campus-carousel-20260916-152420/`；規則在 DESIGN.md「五校分區輪播」。
