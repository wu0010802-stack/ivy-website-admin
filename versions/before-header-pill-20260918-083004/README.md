...
2026-09-17（晚）：導覽列離開首屏後收進漢堡選單的三個互動 mock-up 在 `design/header-collapse-directions/`（A 同組收合、B 浮動膠囊、C 極簡列＋全幕選單），比較頁可切桌面／手機、跳狀態、模擬減少動態；截圖在同目錄 `shots/`，可分享的 Artifact：https://claude.ai/artifact/KwzBCVQSYgew8XB5cmbGhm 。使用者選 B 後另做 `b-tuning.html`：膠囊三檔尺寸×四種色調疊在真實首頁背景條上比較（附對比度），建議深綠 × M。尚未套用主站。
