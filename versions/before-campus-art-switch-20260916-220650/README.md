...
2026-09-16：分校資訊採用 C「校舍輪廓線稿」背景，米白底搭配左下校舍與右下樹木；手機版線稿位於區塊底部。素材 `assets/campus-line-art.webp` 約 64 KB，已內嵌到單檔預覽；320～2048px 響應式、鍵盤選校與預約入口驗證通過。提案與素材來源在 `design/campus-background-mockups/`，設計規則見 `DESIGN.md`。
