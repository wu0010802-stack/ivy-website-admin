...
- 切換：軌道 tab 鍵盤操作、上一／下一校首尾循環、點地圖圖釘；照片預載後 200ms 淡入淡出，減少動態直接換，手動切換時 aria-live 播報目前校區。每 6 秒自動循環五校，提供暫停／播放鈕；滑鼠停留、離開可視範圍或背景分頁時暫停。手動選校、鍵盤焦點進入或觸控操作後停止，需按播放恢復；減少動態效果預設不自動播放。
