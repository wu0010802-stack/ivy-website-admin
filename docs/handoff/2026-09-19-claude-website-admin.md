...
版本：v3，2026-09-19。公開前台採 Nuxt（已定案，不需再詢問）；**分四個階段執行、階段之間有硬閘**。字型子集、原型凍結點、開發代理、Node 版本等限制見下文。

---

你要在本 repo（官網 repo 的 git root 或其 worktree）實作常春藤官網後台。請全程使用繁體中文（台灣用語）。
...
- 確認 git root、分支與工作目錄。非本任務產生的未提交修改一律視為使用者工作。
...
請先確認前置條件都成立，然後開始實作指定階段，並停在該階段閘門。
