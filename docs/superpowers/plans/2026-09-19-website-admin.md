...
**Revision:** v3，2026-09-19。v2 使用者已選定 Nuxt；v3 依同日審查把 11 個任務分成 A–D 四階段、加入階段閘門與第一版範圍縮減，新增 A25 字型涵蓋驗收。舊版 vanilla adapter 與正式單 HTML 打包要求已被取代。
...
- Node 22 LTS（`.nvmrc`＋`engines`）；PostgreSQL 本機 14.x，文件註明測過版本。
- 開工前使用者已 commit baseline 並開 `feature/website-admin` worktree；工作樹不乾淨就停下回報。

---

## 階段與閘門

一次 session 只執行一個階段；階段結束交驗收報告並停下，使用者拍板後才開下一階段。

| 階段 | 任務 | 閘門（全部成立才能進下一階段） |
|---|---|---|
| A | Task 1、2 | 四視口像素比對通過（Task 2 門檻）；缺字檢查報告已交；**使用者親自看過 Nuxt 版並同意**；通過的 commit 記為原型凍結點 |
| B | Task 3、4、5（第一版範圍） | 園方角色能在本機後台改一段文案、換一張圖並發布到 Nuxt 站；Task 3 兩種角色的正負權限測試綠 |
| C | Task 6（不含 slots）、Task 8 | 真實 inquiry 送單、idempotency 重播、`BOOKING_CONFIG_CHANGED`、SSR 新鮮度測試綠；fixture 模式在 production 拒絕啟動 |
| D | Task 7、9、10；Task 3／5 的延後範圍 | 開工前先向園方確認 `slots`；每一塊可再拆或延後 |

Task 11 的驗收表在每階段結束時只填該階段列；其餘標 not-run 並註明階段。
...
- web 的 `dev/build/start/typecheck/test:unit` scripts 必須可用；Node 釘 22 LTS，Nuxt 4 取該 Node 版本支援的最新穩定 patch 並寫入 lockfile。
...
**階段 A。Files:** Create `docs/website-admin/baseline.md`、`backend/pyproject.toml`、`backend/app/{main,config,db}.py`、`backend/tests/conftest.py`、`web/{package.json,nuxt.config.ts}`、`admin/{package.json,vite.config.ts}`、root `package.json`、`playwright.config.ts`、`.nvmrc`（root、web、admin）；Modify `.gitignore`：只追加 `/artifacts/`、`/web/.output/`、`/web/.nuxt/` 與 admin／backend 產物，不整理既有規則。

**Consumes:** 現行官網工作目錄、規格與五校內容。

**Produces:** 啟動／測試 scripts、明確隔離 DB、before 畫面、測試 fixture 契約。

- [ ] 確認前置條件：`git status --short` 乾淨、位於 `feature/website-admin` worktree、`node -v` 為 22.x；任一不成立停下回報。記錄目前 commit 與現用路由。
...
- [ ] 建立開發同源代理：`nuxt.config.ts` 的 `nitro.devProxy` 把 `/api/website/v1/` 轉到 FastAPI；`admin/vite.config.ts` 設 `base: '/admin/'` 與相同 `server.proxy`。不裝 caddy／nginx，不開任意 CORS。
...
**階段 A。Files:** Create `web/app/{app.vue,pages/index.vue,pages/campuses/[key].vue,pages/visit/index.vue,pages/visit/[key].vue}`、`web/app/components/{SiteHeader,HeroVideo,AboutSection,DayExperience,DayMomentCard,CampusBoard,CampusTour,NewsDialog,CampusFaq,VisitForm,SiteFooter}.vue`、`web/app/composables/usePublishedSite.ts`、`web/app/plugins/legacy-hash.client.ts`、`web/app/assets/css/`、`content/site-fixture.json`、`tests/e2e/{nuxt-rendering,legacy-routes}.spec.ts`。
...
- [ ] 視覺比對用 Playwright `toHaveScreenshot`：baseline 取自 Task 1 對原型的截圖，1440／1024／390／375 四視口、首頁與五校頁、reduced-motion 開啟、影片替換為 poster 以避免幀差；`maxDiffPixelRatio` 0.01，超過即失敗並輸出 diff 圖。agent 目視比對不能取代此測試，測試通過後仍須使用者親自看過才算閘門通過。
- [ ] 缺字檢查：用 fontTools 讀 `lineseed-bd.woff`、`lineseed-eb.woff`、`noto-sans-tc-600-brand.woff` 的 cmap，比對 fixture 全部標題與品牌欄位，缺字列進 `docs/website-admin/baseline.md`；不得默默退回系統字。完整字型檔的取得屬階段 B 前置（規格 3.1.1）。
...
**階段 B（第一版只做總管理者、分校管理者；內容編輯／接待／唯讀角色屬階段 D）。Files:** Create `backend/app/auth/*`、`backend/app/campuses/*`、首版 migration、`backend/app/cli.py`、`backend/tests/test_auth_scope.py`、`admin/src/{api,stores,router,layouts}`、`admin/src/views/{LoginView,UsersView}.vue`。
...
- [ ] 完成登入畫面、登入狀態恢復、登出、帳號管理、角色與校區設定；路由隱藏不能取代 API 授權。角色 enum 完整定義但後台第一版只提供兩種選項；`editor_client` 等其他角色 fixture 延至階段 D。
...
**階段 B。Files:** Create `backend/app/media/*`、migration、`backend/tests/test_media.py`、`admin/src/views/MediaLibraryView.vue`、`admin/src/components/{MediaPicker,MediaCropEditor}.vue`、`admin/tests/media.spec.ts`。
...
- [ ] 實作本機 storage adapter、metadata、縮圖與非同步影片處理；檔名轉隨機 key，關閉原始路徑存取。本機 ffmpeg 8.x 無 libwebp：poster 與 WebP 縮圖先由 ffmpeg 抽 PNG 再以 Pillow 轉檔，adapter 不得假設 ffmpeg 能直接輸出 WebP。
- [ ] 完整字型檔到位（規格 3.1.1）：LINE Seed TW Bold／ExtraBold 轉 woff2 放進 `web/`，`@font-face` 改指完整檔；子集 woff 只留給凍結後的原型。
...
- [ ] 完成版本／發布／局部還原。發布交易鎖定現行 release 指標後組合 manifest，切換原子完成。狀態欄位預留審核值但第一版 API 只接受草稿與已發布。
- [ ] （階段 D）審核流程與排程：排程綁定指定 revision；到期下架同樣產生新 release；權限失效、素材未就緒時記失敗，不默默發布。
- [ ] 標題類欄位保存時回傳目前字型涵蓋檢查結果（規格 3.1.1）；完整字型到位後只作資訊提示。
...
**階段 C（`slots` 屬階段 D：enum 保留、後台顯示「尚未開放」不可啟用、公開提交回 `BOOKING_UNAVAILABLE`）。Files:** Create `backend/app/booking/{models,schemas,config_service,request_service,routes}.py`、migration、`backend/tests/test_booking_modes.py`、`web/app/utils/booking-action.ts`、`admin/src/views/BookingSettingsView.vue`、`web/tests/booking-action.spec.ts`。
...
- [ ] 實作公開需求提交：驗證 → idempotency → 設定列鎖 → 重驗模式／version → 存案件／同意／歷程／outbox → commit → 回應。outbox 列在此階段必寫，但 worker 發送屬階段 D。
...
**階段 D（開工前先向園方確認 `slots` 是否需要）。Files:** Create `backend/app/booking/{slot_service,workflow_service}.py`、migration、`backend/tests/{test_booking_concurrency,test_visit_workflow}.py`、`admin/src/views/{VisitRequests,VisitDetail,VisitCalendar,VisitRules}View.vue`。
...
**階段 D。Files:** Create `backend/app/notifications/*`、`backend/app/workers/*`、`backend/app/booking/access_service.py`、`backend/tests/test_notifications.py`、`admin/src/views/NotificationsView.vue`、`web/app/pages/visit/manage.vue`。
...
**階段 D。Files:** Create `backend/app/operations/*`、`backend/tests/test_operations.py`、`admin/src/views/{Dashboard,Analytics,Audit,Policies}View.vue`、`contracts/openapi.json`、`scripts/{export_openapi,backup_website,restore_website}`（使用適合語言副檔名）、`docs/website-admin/{README,operations,acceptance}.md`。
...
**每階段結束各執行一次，只填該階段列。Files:** Modify `docs/website-admin/acceptance.md`、root README（只新增本次啟動連結與必要修正，不覆蓋既有設計紀錄）。
...
| ID | 階段 | 驗收情境 | 證據 |
|---|---|---|---|
| A01 | B | 現有首頁、五校、一天影片與照片卡、探索、消息、FAQ 欄位都有 editor | 後台逐頁測試與欄位對照 |
| A02 | B | 修改一校不影響另一校；role/scope 在 API 生效 | 正負權限整合測試 |
| A03 | B | 草稿不可公開；發布／指定版本還原正確；預約不跟著回滾 | release tests + E2E |
| A04 | B | 圖片／影片／poster 替換、裁切、引用保護、私有素材、熱點複核 | media/content tests |
| A05 | C | 每校六模式切換，缺連結不啟用，原案件仍存在 | booking modes tests |
| A06 | C | 頁首、頁尾、首頁分校、分校頁、探索等所有預約 CTA 一致 | 入口矩陣 E2E |
| A07 | C | 表單成功持久化；失敗保留輸入；重送只建一案 | API + public UI tests |
| A08 | C | 舊 config version 被拒；成功後重播仍回原結果 | transaction race tests |
| A09 | D | 最後一格並發只有一組；取消／改期／到期無超收 | 真 PG concurrency tests |
| A10 | D | 規則、例外日、提前時間、滿額、手動／自動確認 | slot/workflow tests |
| A11 | D | 人工補登、聯絡、承辦、狀態、日曆、匯出同源且有權限 | 接待 E2E + export tests |
| A12 | D | 失敗通知可重試、無重複、案件不丟失、worker 可恢復 | outbox/worker tests |
| A13 | D | 家長只讀自己的案件，安全取消／申請改期／token 過期 | access E2E |
| A14 | D | 點擊和預約分開；Dashboard 與分析不漏校或 PII | operations tests |
| A15 | D | 審核、排程、到期下架、併發編輯與權限失效正確 | publish job tests |
| A16 | C | noindex 預設、SSR title/description/OG/canonical、正式索引開關與 sitemap 排除規則 | 原始 HTML + sitemap 測試 |
| A17 | D | 保存政策 dry-run／匿名化、備份還原有實測 | 隔離資料演練紀錄 |
| A18 | A | 1440/1024/390/375px、鍵盤、降動態、無水平溢出 | before/after 截圖及 E2E |
| A19 | A | 原型快照可離線開啟；新版 Nuxt localhost／授權預覽可用，fixture 不會真實提交 | 快照 checksum/offline + Nuxt preview E2E |
| A20 | B | web/admin 共用 OpenAPI 型別，fresh setup、Nuxt build/start、admin build、測試可重現 | command log 與 dependency locks |
| A21 | A | 五校正式路徑、直接開啟／刷新、前進後退、舊 hash 相容、未知校區 404 | Nuxt route E2E + HTTP status |
| A22 | A | 主要內容在 SSR HTML，禁用 JS 仍可讀；不靠整頁 ClientOnly | raw HTML + no-JS browser |
| A23 | C | 發布後新 SSR／刷新／站內換頁讀新 release；hydrate 不重複讀取；無跨 request 私密資料 | release freshness 與 scope tests |
| A24 | A | 無 hydration mismatch，進出頁清理動畫／影片；私有資產與原始碼不被靜態服務暴露 | console/lifecycle + HTTP 邊界測試 |
| A25 | A→B | 階段 A：缺字檢查報告完整；階段 B：完整字型檔到位，後台任意輸入的標題不退回系統字 | fontTools cmap 比對 + 隨機標題截圖 |

- [ ] 對每一列記錄 pass/fail/not-run、測試名稱或檔案路徑；不屬本階段者標 not-run 並註明階段；缺外部設定與已知限制另列。
- [ ] 階段 A 結束另記原型凍結點 commit hash；階段 B 起確認根目錄 vanilla 檔案未被修改。
