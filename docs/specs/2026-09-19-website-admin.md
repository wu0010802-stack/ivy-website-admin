...
> 2026-09-22 單項需求更新：園方依現有預約截圖要求日期／場次、孩子姓名／生日、Email 與得知管道，已採 A 版整合。下文 v3「不蒐集兒童姓名／生日」及 slots 未確認的限制由本次裁定取代；不代表其他延後功能一併啟動。各校仍須自行設定 slots 模式與可用場次，未自動對外開放。新版前台要求孩子姓名／生日，Email／來源為選填；API 新欄位保持 nullable／空清單以相容既有案件與舊請求。資料、CSV、保存政策及型別契約同步，既有預約確認與容量規則繼續適用。
