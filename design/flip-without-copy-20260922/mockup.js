const directions = [
  { key: 'a', title: '掀起一角', subtitle: '把「背面」直接露出來', tag: '已選定・微調', description: '<strong>用紙張本身邀請點擊。</strong><br>32px 淡折角、微微露出橫線；折角跟著相片一起轉動，不留在原位。' },
...
function cue(key) {
  if (key === 'a') return `<span class="cue curl" aria-hidden="true"><svg viewBox="0 0 100 100"><path class="curl-paper" d="M0 0Q48 7 100 0L0 100Q7 48 0 0Z"/><path class="curl-line" d="M2 21H79M3 42H58M3 63H37M2 84H16"/></svg></span>`;
...
document.querySelector('.directions').innerHTML = directions.map(d => `
  <article class="direction" data-direction="${d.key}" aria-labelledby="heading-${d.key}">
    ...
    <div class="photo-stage"><div class="card variant-${d.key}">
      <div class="rotator">
        <div class="face front"><figure class="photo"><img src="../../web/public/assets/day-hello.webp" width="810" height="600" alt="孩子在熟悉的環境裡，與身旁的人互動"><figcaption>陪伴互動 · 入園情境示意</figcaption><time datetime="08:00">08:00</time></figure><p class="kicker">01 / 早安入園</p><h3>早安，今天的我<br>準備好了。</h3>${d.key === 'a' ? cue(d.key) : ''}</div>
        <div class="face back" id="story-${d.key}" inert><p class="kicker">01 / 早安入園</p><p class="story">和家人說聲再見，走進熟悉的校園。從整理小書包開始，一點一點，找到自己的步調。</p><p class="question">孩子第一次上學，如何陪伴適應？</p><p class="answer">參觀時可以與園所聊聊：初次入園的陪伴方式、家長如何與老師聯繫，以及可以事先做哪些準備。</p>${d.key === 'a' ? cue(d.key) : ''}</div>
      </div>
      ${d.key !== 'a' ? cue(d.key) : ''}
