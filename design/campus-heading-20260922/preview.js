...
async function init() {
  ...
  campuses.forEach((c, i) => {
    ...
    tab.setAttribute('aria-controls', 'campus-details'); tab.dataset.number = String(i + 1).padStart(2, '0'); tab.textContent = c.name;
  ...
  select(4); renderPlayback(); requestAnimationFrame(tick);
}
init().catch(error => {
