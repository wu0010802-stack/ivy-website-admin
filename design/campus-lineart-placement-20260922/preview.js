import { buildingOutlines } from './building-outlines.js';
...
async function init() {
  ...
  let index = 0, paused = true, focused = false, visible = true, elapsed = 0, previousTime = 0, pointerStart = null;

  campuses.forEach((c, i) => {
    const tab = document.createElement('button');
    tab.type = 'button'; tab.id = `campus-tab-${c.key}`; tab.role = 'tab'; tab.setAttribute('aria-label', c.name);
    tab.setAttribute('aria-controls', 'campus-details'); tab.dataset.number = String(i + 1).padStart(2, '0');
    tab.innerHTML = `<span class="tab-building" aria-hidden="true"><img class="tab-art" src="../../web/public/assets/campus-line-art-${escapeHtml(c.key)}.webp" width="1536" height="1024" alt=""><svg class="building-outline" viewBox="0 0 1536 1024" aria-hidden="true"><path d="${buildingOutlines[c.key] || ''}" vector-effect="non-scaling-stroke"/></svg></span><span class="tab-label">${escapeHtml(c.name)}</span>`;
  ...
  function select(next, automatic = false) {
    ...
    root.querySelector('.corner-art').src = `../../web/public/assets/campus-line-art-${c.key}.webp`;
    ...
    details.innerHTML = `<div class="campus-identity"><span class="campus-district">高雄 · ${escapeHtml(c.district)}</span><h3><a href="${campusUrl}">${escapeHtml(c.name)}</a></h3><span lang="en">${escapeHtml(c.key.toUpperCase())} CAMPUS</span></div>
  ...
  function tick(time) {
    ...
    const playing = !paused && !document.hidden && visible && !focused;
  ...
  const requested = new URLSearchParams(location.search).get('campus') || 'minghua';
  select(Math.max(0, campuses.findIndex(c => c.key === requested))); renderPlayback(); requestAnimationFrame(tick);
}
init().catch(error => {
